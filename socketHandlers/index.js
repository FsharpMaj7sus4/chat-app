...
const makeChatListAndJoin = async (socket, roomsData, userId) => {
	...
	chatList.sort((a, b) => {
		const aLastUpdate = a.lastMessage
			? a.lastMessage.createdAt
			: a.createdAt
		const bLastUpdate = b.lastMessage
...
io.on("connection", async socket => {
	try {
		...
		const roomsData = user.dataValues.Rooms.map(room =>
			room.get({ plain: true })
		)
		...
		socket.on("newTextMessage", async data => {
			...
			let newMessage = await Message.create(messageInfo).then(message =>
		...
		socket.on("newFileMessage", async data => {
			...
			const file = await File.create(fileInfo).then(file =>
				file.get({ plain: true })
			)
			const messageInfo = Object.assign(
				{},
		...
		socket.on("seen", async roomId => {
			await Message.update(
				{ isSeen: true },
				{
					where: {
						RoomId: roomId,
						senderId: { [Sequelize.Op.not]: user.id },
					},
				}
			)
		...
		socket.on("newPvRoom", async otherUserId => {
			...
			if (created) {
				...
				if (connectedUsers[otherUserId]) {
					io.sockets.connected[connectedUsers[otherUserId]].join(
						room.id
					)
					io.to(connectedUsers[otherUserId]).emit(
						"addedToNewRoom",
						room
					)
				}
